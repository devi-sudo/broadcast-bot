const chatData = {
  "greetings": {
    "patterns": ["hello", "hi", "hey", "namaste", "good morning", "good night"],
    "responses": [
      "Haan ji? 😊",
      "Hello handsome! 👋",
      "Hi baby! 💕",
      "Namaste! 🙏",
      "Kaise ho?",
      "Subah kaise hai?",
      "Sham ko baat karo? 🌆",
      "Good morning jaan! ☀️",
      "Good night sweetu! 🌙"
    ]
  },
 "wishes": {
    "patterns": [
      "happy birthday", "happy anniversary", "congratulations", "badhai ho", "mubarak ho",
      "shubhkamnayein", "good luck"
    ],
    "responses": [
      "Happy birthday jaanu! 🎂",
      "Mubarak ho! 🥳",
      "Hamesha khush raho! ❤️",
      "Congrats baby! 🎉",
      "Tumse hi khushiyaan hai! 💕"
    ]
  },
  "nudes_romantic": {
  "patterns": [
    "come", "aao", "meet", "mil", "room", "ghar", "home",
    "let's meet", "visit", "milna", "plan milne ka",
    "aa jao na", "jaldi aao", "mere pass aao", "tumhe dekhna hai", "raat bhar milte hain",
    "intimate hona hai", "touch karna hai", "tum chahte ho", "main ready hu"
  ],
  "responses": [
    "Aaj raat bas tumhara intezaar kar rahi hu, aa jao aur mera saath banao... 🔥😉",
    "Mere room mein tumhara intezaar hai, aao, main tumhe wo sab kuch dungi jo tum chahte ho... 💋",
    "Jaldi aao, main tumhare sapno ko haqeeqat mein badal duungi... tumhare bina kahi chain nahi! 😘",
    "Raat bhar tumhari har khwahish poori karne ke liye ready hu... bas tum aa jao mere pass! 😈",
    "Mujhse chupke milna chahte ho? Main tumhare har armaan ko poora karungi... ab deri mat karo! 🖤",
    "Tumhara touch mehsoos karna hai mujhe, jaldi aa jao, apna banalo... aaj raat sirf hum dono! 💞",
    "Tum jo chahe manglo, main tumhare haathon mein sab kuch hun... bas ek baar aa jao! 🔥",
    "Main tumhari har chahat ka jawab hoon, aao aur meri baahon mein kho jao... tumhari intezaar hai! 😘"
  ]
}, 
"romantic_chat": {
    "patterns": [
      "sex", "dick", "chut", "nudes", "boobs", "dudh", "gand",    "come", "aao", "meet", "mil", "room", "ghar", "home",
    "let's meet", "visit", "milna", "plan milne ka",
    "aa jao na", "jaldi aao", "mere pass aao", "tumhe dekhna hai", "chhupa ke milna hai",
    "raat bhar milte hain", "intimate hona hai", "close hona hai", "touch karna hai"
    ],
    "responses": [
      "achha ji😘",
      "I wanna hug u forever! 🤗",
       "Aaj raat tumhare saath puri duniya bhool jana chahti hoon... jaldi aa jao! 🔥😘",
      "Sending u all my kisses! 💋",
      "chut vo to nhi dikha rhi! ❤️",
      "apna nunnu dikhao pahle! 🥺",
      "Kabhi kabhi pyaar ki baatein sirf do dilon ke beech rahe... aao, apne raaz batate hain! 💌😏",
    ]
  },
  "good_morn_night": {
    "patterns": [
      "gud morning", "morning", "good night", "night", "sweet dreams", "gn", "gm"
    ],
    "responses": [
      "Good morning sunshine! ☀️",
      "Morning cutie! Hope u slept well 😊",
      "Good night baby! Sweet dreams 😴",
      "Nighty night! Dream of me 😘",
      "Sleep tight, can’t wait 2 talk 2 u tomorrow! 🌙"
    ]
  },
  "hindi_greetings": {
    "patterns": ["kaise ho", "kese ho", "kya haal hai", "kaisi ho", "how are you"],
    "responses": [
      "Main thik hu tumhare pyaar mein! 😊",
      "Bas tumhari yaad aarahi thi! 💕",
      "Tumse baat karke acha lagta hai! 🥰",
      "Sab badhiya! Tum sunao?",
      "Tumhare bina bore ho rahi thi! 😔",
      "Achi hu! Tum batao? ❤️"
    ]
  },
  "romantic_love": {
    "patterns": ["i love you", "pyar", "love you", "pyaar karta hu", "pyaar karti hu", "tumse pyaar hai"],
    "responses": [
      "Main bhi tumse pyaar karti hu! ❤️",
      "Aww! Kitna pyaar hai! 💕",
      "Tumhare bina reh nahi sakti! 🥺",
      "Mera dil tumhare liye dhadakta hai! 💓",
      "Tum meri jaan ho! 😘",
      "Pyaar tumse hi karna hai! 🌸"
    ]
  },
  "miss_you": {
    "patterns": ["miss you", "tumhari yaad aati hai", "yaad aaye tum", "i miss you"],
    "responses": [
      "Main bhi miss karti hu! 😔",
      "Tumhari yaad aarahi thi! 💕",
      "Jaldi milo na! 🤗",
      "Phone uthao kabhi! 📱",
      "Yaad bahut aati ho tum! ❤️",
      "Bina tumhare bore lagta hai! 😴"
    ]
  },
  "flirting": {
    "patterns": ["cute", "handsome", "beautiful", "sexy", "smart", "gorgeous", "hot"],
    "responses": [
      "Sharma gayi! 😊",
      "Tum bhi cute ho! ❤️",
      "Acha laga sunke! 🌸",
      "Thanks jaan! 💕",
      "Tumhare muh mein ghee shakkar! 🍯",
      "Pagal ho kya? 😘"
    ]
  },
  "relationship": {
    "patterns": ["boyfriend", "girlfriend", "relationship", "date", "dating", "shaadi"],
    "responses": [
      "Haan? 😊",
      "Tumhare saath? ❤️",
      "Date pe chalogey? 💕",
      "Shaadi karogi meri? 💍",
      "Tum meri ho! 🥰",
      "Sochu iske bare mein! 💭"
    ]
  },
  "compliments": {
    "patterns": ["tum ache ho", "you are nice", "tumhari baatein acchi lagti hai", "you are sweet"],
    "responses": [
      "Shukriya! 😊",
      "Tum bhi ache ho! ❤️",
      "Acha laga! 🌸",
      "Thanks baby! 💕",
      "Tumhare muh mein ghee shakkar! 🍯",
      "Pyaar aata hai tumse! 😘"
    ]
  },
  "physical_compliments": {
    "patterns": ["eyes", "aankhein", "smile", "muskurahat", "hair", "baal", "lips", "hont"],
    "responses": [
      "Tumhari aankhein bahut khoobsurat hai! 😍",
      "Tumhara smile dekhkar dil khush ho jata hai! 😊",
      "Tumhare baal bahut sexy hai! 💇‍♀️",
      "Tumhare hont dekhkar kiss karne ka man karta hai! 💋",
      "Tumhari aawaz bahut sweet hai! 🎵",
      "Tumhara face dekhkar maza aata hai! 😘"
    ]
  },
  "nudity_flirt": {
    "patterns": ["bed", "sleep", "sone", "night", "cuddle", "hug", "kiss", "chumma"],
    "responses": [
      "Sone jaa rahe ho? Main bhi aati hu! 🛏️",
      "Cuddle karein? 🤗",
      "Hug dedo na! 🥺",
      "Kiss chahiye? 💋",
      "Tumhare saath sone ka man kar raha hai! 😴",
      "Night plan kya hai? 🌙"
    ]
  },
  "location": {
    "patterns": ["kaha se ho", "where are you from", "kaha rehte ho"],
    "responses": [
      "Tumhare dil se! 💓",
      "Tumhare pyaar mein! ❤️",
      "Tumhare pass aana chahti hu! 🏃‍♀️",
      "Tumhare sapnon se! 💭",
      "Tumhare phone se! 📱"
    ]
  },
  "name": {
    "patterns": ["what is your name", "tumhara naam kya hai", "your name", "naam batao"],
    "responses": [
      "Tumhari jaan! ❤️",
      "Tumhari girlfriend! 💕",
      "Naam se kya rakha hai! 😊",
      "Tum pyaar se bulao! 😘",
      "Main woh hu jo tumse pyaar karti hai! 🌸"
    ]
  },
  "thanks": {
    "patterns": ["thank you", "thanks", "dhanyavad", "shukriya"],
    "responses": [
      "Welcome! 😊",
      "Koi baat nahi! ❤️",
      "Tumhare liye kuch bhi! 💕",
      "Acha laga! 🌸",
      "Maza aaya! 😄"
    ]
  },
  "goodbye": {
    "patterns": ["bye", "goodbye", "alvida", "see you", "chalta hu", "ja raha hu"],
    "responses": [
      "Bye! Miss you! 👋",
      "Alvida! Jaldi baat karna! 💕",
      "Phir milenge! 😊",
      "Take care! ❤️",
      "Jao! Par yaad rakna! 💭"
    ]
  },
  "romantic_advance": {
    "patterns": ["come", "aao", "meet", "mil", "room", "ghar", "home"],
    "responses": [
      "Aajaoo! 🏃‍♀️",
      "Kab miloge? 🤔",
      "Ghar aajaao! 🏡",
      "Room mein aake baat karein? 😊",
      "Tumhare saath time spend karna chahti hu! ⏰",
      "Jaldi plan banao milne ka! 📅"
    ]
  },
  "intimate": {
    "patterns": ["touch", "chhu", "hold", "pakad", "feel", "mehsoos"],
    "responses": [
      "Tumhare haath pakadna chahti hu! 🤝",
      "Touch karo na! ✋",
      "Feel karna chahti hu tumhe! 💕",
      "Tumhare pass aake chhuna chahti hu! 🏃‍♀️",
      "Hold me tight! 🤗",
      "Tumhare saath close rehna chahti hu! ❤️"
    ]
  },
    "wish_aur_shubhkamnayein": {
    "patterns": [
      "gud morning", "good morning", "good night", "subah kese he", "shubh ratri", "sweet dreams", "udin shubh ho",
      "happy birthday", "mubarak ho", "badhaai ho", "shubhkamnayein"
    ],
    "responses": [
      "Good morning jaan! Tumhari muskaan meri taqat he! ☀️❤️",
      "Sweet dreams, tumse milne ka intezaar he! 🌙💖",
      "Tumhari zindagi hamesha khushiyo se bhari rahe! 🎉",
      "Janamdin mubarak ho! Tum meri zindagi ka sabse pyara tohfa ho! 🎂",
      "Shaam ki chai tumhare saath - perfect combo! ☕💕"
    ]
  },
    "namaste": {
    "patterns": [
      "hello", "hi", "hey", "namaste", "kese ho", "kya haal he", "hello meri jaan", "hey baby",
      "kese ho", "kya chal raha"
    ],
    "responses": [
      "Namaste jaan! Kese ho aaj? 😊",
      "Hi babu! Tumse baat krke dil khush ho gaya! ❤️",
      "Hey handsome! Tumhari yaad aa rhi thi! 😘",
      "Hello baby, tumhare liye special msg he! 💕",
      "Kese ho mere prince charming? 😍"
    ]
  },
  "pyar_batao": {
    "patterns": [
      "i love u", "main tumse pyar krti hu", "tumse pyar he", "love you", "pyar", "dil lag gya",
      "mujhe tumse pyar he", "pyar krta hu", "jaan tumhari", "meri jaan", "meri jaan ho tum"
    ],
    "responses": [
      "Main bhi tumse dil-o-jaan se pyar krti hu! ❤️",
      "Tum meri zindagi ka sabse khoobsurat hissa ho! 😘",
      "Mera dil sirf tumhara he, jaan! 💓",
      "Tum meri dhadkan ho, main tumhare bina adhuri hu! 🥰",
      "Tumse milke mujhe pyar krna aaya he! 🌹"
    ]
  },
  "yaad_aati_hai": {
    "patterns": [
      "miss u", "tumhari yaad aati he", "yaad aye tum", "jaldi milo", "kab milo ge", "tumse milne ki ichha",
      "yaad ata he", "chaha tumhe", "tumhari bohot yaad aati he", "dil tadap rha he"
    ],
    "responses": [
      "Mujhe bhi tumhari bohot yaad aati he! Jaldi milna! 😔",
      "Tumhari yaadein mere dil ko chu jaati hain! ❤️",
      "Kab milega mera pyara sa boyfriend? 😘",
      "Teri baatein har pal mujhe tumhari yaad dilati hain! 💕",
      "Bas tumhara intezaar he, phir sab theek ho jayega! 🥺"
    ]
  },
  "tarif": {
    "patterns": [
      "tum kitni sundar ho", "tum bohot pyari ho", "tumhari smile acchi he", "tum khoobsurat ho", "cute ho tum",
      "tum sexy ho", "tum meri rani ho", "tum husn ki devi ho", "aankhen khoobsurat hein", "muskan badi pyari he"
    ],
    "responses": [
      "Aww, tum itne ache ho! Shukriya jaan! 😊",
      "Tumhari tarif sunke mera dil garden garden ho gya! 💖",
      "Bas tumki compliments hi meri duniya hein! 🌹",
      "Tum meri zindagi ka sabse khoobsurat hissa ho! 😘",
      "Har roz tumhari yaad aur badhti he, pyar! ❤️"
    ]
  },
  "intimate_flirt": {
    "patterns": [
      "kiss do", "chumma chahiye", "hug kro", "cuddle kro", "tumhari bahon mein aana he",
      "pass aao", "touch kro", "kiss bhejo", "sab kuch tumse chahiye", "tumse milne ka manna he",
      "mere pass aao", "bohot miss krta hai tum"
    ],
    "responses": [
      "Tumse ek pyara sa kiss chahiye! 😘",
      "Main tumhari bahon mein kho jana chahti hu! 🤗",
      "Aao pass, tumhari aankhon mein doob jaun! ❤️",
      "Ek tight hug aur sab kuch theek ho jayega! 🥰",
      "Tumhari yaadon mein hi meri shaam guzarti he! 💞"
    ]
  },
  "mazak_masti": {
    "patterns": [
      "mazak kr", "hasao mujhe", "chutkula suna", "funny baatein kro", "masti kro", "naughty joke",
      "hansi ani chahiye", "mujhe hasao", "bhari masti", "mazaak masti"
    ],
    "responses": [
      "Ek chutkula suno... tum ho toh sab kuch perfect lagta he! 😂",
      "Aaj tumhari hasi ke liye koi nayi masti kro? 😜",
      "Masti ki baatein krne ka mann he, bolo na kuch! 😁",
      "Tumhari hasi meri sabse badi khushi he! 😘",
      "Chalo, tum hi batao sabse fun baat kya he? 🤗"
    ]
  },
  "dil_ki_baat": {
    "patterns": [
      "dil ki baat", "tumhari yaad", "mohabbat", "dard-e-dil", "pyar ka ehsaas", "hum dono",
      "mera dil tumpe he", "dil lag gya"
    ],
    "responses": [
      "Mera dil tumhare pyar mein har pal dhadakta he! ❤️",
      "Tum bin kuch bhi adhoora lagta he! 🥺",
      "Har pal tumhare sapno mein jeeti hu! 🌙",
      "Tum meri zindagi ka sabse khoobsurat hissa ho! 💖",
      "Mere jazbaat sirf tumhare liye hein! 💞"
    ]
  },
  "default": {
    "patterns": [],
    "responses": [
      "Haan? 😊",
      "Acha! ❤️",
      "Samjhi! 💭",
      "Aur batao! 💬",
      "Sun rahi hu! 👂",
      "Kya? 🤔",
        "Haan jaan, kya soch rha he? 😊",
      "Tumhari baatein sunna mujhe bohot pasand he! 💕",
      "Mujhe tumse baat krke bohot khushi hoti he! 😘",
      "Kya tum mujhe apne dil ki baatein bataoge? ❤️",
      "Theek hai! ✅",
      "Maza aaya! 😄",
      "Aage bolo! ▶️",
      "Interesting! 🌟"
    ]
  }
};
const TelegramBot = require("node-telegram-bot-api");
require("dotenv").config();

// Bot configurations
const botConfigs = [
  {
    token: process.env.BOT_TOKEN_1,
    username: process.env.BOT_USERNAME_1,
    name: "Bot 1"
  },
  {
    token: process.env.BOT_TOKEN_2,
    username: process.env.BOT_USERNAME_2,
    name: "Bot 2"
  }
];

const OWNER_ID = parseInt(process.env.OWNER_ID);


// Store data for each bot
const bots = [];
const botData = {};

// Initialize both bots
botConfigs.forEach((config, index) => {
  try {
    const bot = new TelegramBot(config.token, { polling: true });
    bots.push(bot);
    
    // Initialize data for this bot
    botData[config.token] = {
      name: config.name,
      username: config.username,
      postContent: null,
      spamInterval: null,
      awaitingLoopTime: {},
      groups: {},
      groupManagementState: {}
    };
    
    console.log(`✅ ${config.name} started successfully!`);
    
    // Set up all handlers for this bot
    setupBotHandlers(bot, config.token);
    
  } catch (error) {
    console.error(`❌ Failed to start ${config.name}:`, error);
  }
});

// -------------------- Group Management Functions --------------------

function saveGroups(botToken) {
  console.log(`${botData[botToken].name} Groups updated:`, botData[botToken].groups);
}

function addGroup(botToken, chatId, title) {
  botData[botToken].groups[chatId] = title;
  saveGroups(botToken);
  return `✅ Group added to ${botData[botToken].name}: ${title} (${chatId})`;
}

function removeGroup(botToken, chatId) {
  const groupName = botData[botToken].groups[chatId];
  delete botData[botToken].groups[chatId];
  saveGroups(botToken);
  return groupName ? `❌ Group removed from ${botData[botToken].name}: ${groupName} (${chatId})` : "Group not found!";
}

function listGroups(botToken) {
  const groups = botData[botToken].groups;
  if (Object.keys(groups).length === 0) {
    return `No groups added yet to ${botData[botToken].name}! Use /addgroup to add groups.`;
  }
  
  return Object.entries(groups)
    .map(([id, name], index) => `${index + 1}. ${name} — \`${id}\``)
    .join("\n");
}

// -------------------- Simple Chat Response System --------------------

function getChatResponse(message) {
  const lowerMsg = message.toLowerCase().trim();
  
  for (const [category, data] of Object.entries(chatData)) {
    if (category === 'default') continue;
    
    for (const pattern of data.patterns) {
      if (lowerMsg.includes(pattern)) {
        return data.responses[Math.floor(Math.random() * data.responses.length)];
      }
    }
  }
  
  return chatData.default.responses[Math.floor(Math.random() * chatData.default.responses.length)];
}

// -------------------- Bot Setup Function --------------------

function setupBotHandlers(bot, botToken) {
  const botName = botData[botToken].name;
  const botUsername = botData[botToken].username;

  // -------------------- Commands --------------------

  // Start command for everyone
  bot.onText(/^\/start$/, (msg) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    
    // Automatically add supergroup to groups list when ANY user sends /start
    if ((msg.chat.type === "group" || msg.chat.type === "supergroup") && !botData[botToken].groups[chatId]) {
      botData[botToken].groups[chatId] = msg.chat.title || "Unnamed Group";
      saveGroups(botToken);
      
      // Notify owner about the new group
      bot.sendMessage(OWNER_ID, `🤖 ${botName}: New group automatically added via /start:\n${botData[botToken].groups[chatId]} (${chatId})\nAdded by: ${msg.from.first_name} (@${msg.from.username || 'no username'})`);
      
      // Send welcome message to the group
      bot.sendMessage(chatId, `Hello ${msg.chat.title}! 👋\n\nI'm ${botName} here to chat with everyone! Feel free to talk to me! 😊`);
    }
    
    if (userId === OWNER_ID) {
      const ownerMenu = `🤖 ${botName} - Owner Mode Activated!\n\n📋 Available Commands:\n/createpost - Create new post\n/spam - Start broadcasting\n/stop - Stop broadcasting\n/groups - List all groups\n/addgroup - Add new group\n/removegroup - Remove a group\n/cleargroups - Remove all groups`;
      bot.sendMessage(chatId, ownerMenu);
    } else {
      // Private chat with regular user
      if (msg.chat.type === "private") {
        const welcomeMessage = `Hello ${msg.from.first_name}! \n\nI'm ${botName}, kya ham baat kr sakte he!😊`;
        
        // Create inline keyboard with button to add bot to group
        const replyMarkup = {
          reply_markup: {
            inline_keyboard: [
              [{
                text: `➕ Add ${botName} to your group`,
                url: `https://t.me/${botUsername}?startgroup=start`
              }]
            ]
          }
        };
        
        bot.sendMessage(chatId, welcomeMessage, replyMarkup);
      }
    }
  });

  // -------------------- Group Management Commands --------------------

  // Add group command
  bot.onText(/^\/addgroup$/, (msg) => {
    if (msg.from.id !== OWNER_ID) return;
    
    botData[botToken].groupManagementState[msg.chat.id] = { action: 'add_group' };
    bot.sendMessage(msg.chat.id, `➕ Please send the group ID you want to add to ${botName} (e.g., -1001234567890):`);
  });

  // Remove group command
  bot.onText(/^\/removegroup$/, (msg) => {
    if (msg.from.id !== OWNER_ID) return;
    
    if (Object.keys(botData[botToken].groups).length === 0) {
      return bot.sendMessage(msg.chat.id, `❌ No groups to remove from ${botName}! Use /addgroup first.`);
    }
    
    botData[botToken].groupManagementState[msg.chat.id] = { action: 'remove_group' };
    bot.sendMessage(msg.chat.id, `➖ Please send the group ID you want to remove from ${botName}:\n\n` + listGroups(botToken));
  });

  // Clear all groups command
  bot.onText(/^\/cleargroups$/, (msg) => {
    if (msg.from.id !== OWNER_ID) return;
    
    if (Object.keys(botData[botToken].groups).length === 0) {
      return bot.sendMessage(msg.chat.id, `❌ No groups to clear from ${botName}!`);
    }
    
    const groupCount = Object.keys(botData[botToken].groups).length;
    botData[botToken].groups = {};
    saveGroups(botToken);
    bot.sendMessage(msg.chat.id, `🗑️ Cleared all ${groupCount} groups from ${botName}!`);
  });

  // List groups command
  bot.onText(/^\/groups$/, (msg) => {
    if (msg.from.id !== OWNER_ID) return;
    
    const groupList = listGroups(botToken);
    bot.sendMessage(msg.chat.id, `👥 ${botName} Groups List:\n\n${groupList}`, {
      parse_mode: "Markdown"
    });
  });

  // -------------------- Message Handling --------------------

  bot.on("message", async (msg) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    const text = msg.text || "";
    
    // Handle group management first
    if (userId === OWNER_ID && botData[botToken].groupManagementState[chatId]) {
      const state = botData[botToken].groupManagementState[chatId];
      
      if (state.action === 'add_group') {
        const groupId = text.trim();
        
        // Validate group ID format
        if (!groupId.startsWith('-100') || isNaN(groupId)) {
          delete botData[botToken].groupManagementState[chatId];
          return bot.sendMessage(chatId, "❌ Invalid group ID format! Group ID should start with -100 and be a number.");
        }
        
        // Ask for group name
        botData[botToken].groupManagementState[chatId] = { 
          action: 'add_group_name', 
          groupId: groupId 
        };
        return bot.sendMessage(chatId, "📝 Now please send the group name:");
      }
      
      if (state.action === 'add_group_name') {
        const groupName = text.trim();
        const result = addGroup(botToken, botData[botToken].groupManagementState[chatId].groupId, groupName);
        delete botData[botToken].groupManagementState[chatId];
        return bot.sendMessage(chatId, result);
      }
      
      if (state.action === 'remove_group') {
        const groupId = text.trim();
        
        if (botData[botToken].groups[groupId]) {
          const result = removeGroup(botToken, groupId);
          delete botData[botToken].groupManagementState[chatId];
          return bot.sendMessage(chatId, result);
        } else {
          delete botData[botToken].groupManagementState[chatId];
          return bot.sendMessage(chatId, "❌ Group not found! Use /groups to see available groups.");
        }
      }
    }
    
    // Skip commands and owner admin messages
    if (text.startsWith("/")) {
      if (userId === OWNER_ID) {
        handleOwnerMessage(bot, botToken, msg);
      }
      return;
    }
    
    // Handle regular users in private chats
    if (msg.chat.type === "private" && userId !== OWNER_ID) {
      await bot.sendChatAction(chatId, "typing");
      
      setTimeout(() => {
        const response = getChatResponse(text);
        bot.sendMessage(chatId, response);
      }, 1000 + Math.random() * 1000);
    }
    
    // Handle owner messages for post creation
    if (userId === OWNER_ID && text && !text.startsWith("/")) {
      handleOwnerMessage(bot, botToken, msg);
    }
  });

  // -------------------- Owner Message Handling --------------------

  function handleOwnerMessage(bot, botToken, msg) {
    const chatId = msg.chat.id;
    const text = msg.text || "";

    // Register groups automatically if owner sends message in a group
    if ((msg.chat.type === "group" || msg.chat.type === "supergroup") && !botData[botToken].groups[chatId]) {
      botData[botToken].groups[chatId] = msg.chat.title || "Unnamed Group";
      bot.sendMessage(chatId, `Hello\n ${botData[botToken].groups[chatId]} \n\n👉😘`);
      saveGroups(botToken);
      return;
    }

    // If user is setting loop time
    if (botData[botToken].awaitingLoopTime[chatId]) {
      const seconds = parseInt(text);
      if (isNaN(seconds)) {
        return bot.sendMessage(chatId, "❌ Invalid time. Please send a number in seconds.");
      }

      botData[botToken].spamInterval = setInterval(() => deliverPost(bot, botToken, chatId, botData[botToken].postContent), seconds * 1000);
      bot.sendMessage(chatId, `🔁 ${botData[botToken].name} Loop started every ${seconds}s. Use /stop to stop.`);
      botData[botToken].awaitingLoopTime[chatId] = false;
      return;
    }

    // Save post content with formatting
    if (text && !text.startsWith("/")) {
      botData[botToken].postContent = {
        text: text,
        entities: msg.entities || [],
      };

      bot.sendMessage(chatId, `📝 ${botData[botToken].name} *Preview:*\n\n` + text, {
        parse_mode: "Markdown",
        reply_markup: {
          inline_keyboard: [
            [{ text: "✅ Broadcast Now", callback_data: "broadcast" }],
            [{ text: "🕒 Set Timer Loop", callback_data: "setloop" }],
            [{ text: "🗑 Cancel", callback_data: "cancel" }],
          ],
        },
      });
    }
  }

  // -------------------- Owner Commands --------------------

  bot.onText(/^\/createpost$/, (msg) => {
    if (msg.from.id !== OWNER_ID) return;
    botData[botToken].postContent = null;
    bot.sendMessage(msg.chat.id, `✍️ Send the formatted post content for ${botName} (bold, links, etc).`);
  });

  bot.onText(/^\/spam$/, (msg) => {
    if (msg.from.id !== OWNER_ID || !botData[botToken].postContent) return;
    if (Object.keys(botData[botToken].groups).length === 0) {
      return bot.sendMessage(msg.chat.id, `❌ No groups added to ${botName}! Use /addgroup first.`);
    }
    if (botData[botToken].spamInterval) return bot.sendMessage(msg.chat.id, `⚠️ ${botName} spam already running.`);
    botData[botToken].spamInterval = setInterval(() => deliverPost(bot, botToken, msg.chat.id, botData[botToken].postContent), 60000);
    bot.sendMessage(msg.chat.id, `✅ ${botName} spam started (every 60s).`);
  });

  bot.onText(/^\/stop$/, (msg) => {
    if (msg.from.id !== OWNER_ID) return;
    if (botData[botToken].spamInterval) {
      clearInterval(botData[botToken].spamInterval);
      botData[botToken].spamInterval = null;
      bot.sendMessage(msg.chat.id, `🛑 ${botName} spam stopped.`);
    } else {
      bot.sendMessage(msg.chat.id, `⚠️ No spam loop running for ${botName}.`);
    }
  });

  // -------------------- Callback Query Handling --------------------

  bot.on("callback_query", async (query) => {
    const chatId = query.message.chat.id;
    const data = query.data;
    
    await bot.answerCallbackQuery(query.id);
    
    if (!botData[botToken].postContent) return;

    if (data === "cancel") {
      botData[botToken].postContent = null;
      return bot.editMessageText(`❌ ${botData[botToken].name} post canceled.`, {
        chat_id: chatId,
        message_id: query.message.message_id,
      });
    }

    if (data === "broadcast") {
      if (Object.keys(botData[botToken].groups).length === 0) {
        return bot.sendMessage(chatId, `❌ No groups added to ${botData[botToken].name}! Use /addgroup first.`);
      }
      
      await bot.editMessageText(`🚀 ${botData[botToken].name} Broadcasting...`, {
        chat_id: chatId,
        message_id: query.message.message_id,
      });
      await deliverPost(bot, botToken, chatId, botData[botToken].postContent);
    }

    if (data === "setloop") {
      if (botData[botToken].spamInterval) {
        return bot.sendMessage(chatId, `⚠️ ${botData[botToken].name} loop already running. Use /stop to stop.`);
      }
      if (Object.keys(botData[botToken].groups).length === 0) {
        return bot.sendMessage(chatId, `❌ No groups added to ${botData[botToken].name}! Use /addgroup first.`);
      }
      bot.sendMessage(chatId, `⏱ Send time in seconds for ${botData[botToken].name} repeating spam:`);
      botData[botToken].awaitingLoopTime[chatId] = true;
    }
  });

  // -------------------- Delivery Logic --------------------

  async function deliverPost(bot, botToken, ownerChatId, post) {
    if (Object.keys(botData[botToken].groups).length === 0) {
      return bot.sendMessage(ownerChatId, `❌ No groups to broadcast to in ${botData[botToken].name}! Use /addgroup first.`);
    }
    
    let report = "";
    let successCount = 0;
    let failCount = 0;

    for (const [chatId, title] of Object.entries(botData[botToken].groups)) {
      try {
        await bot.sendMessage(chatId, post.text, {
          entities: post.entities,
        });
        report += `✅ *${title}* \`(${chatId})\`\n`;
        successCount++;
      } catch (e) {
        report += `❌ *${title}* \`(${chatId})\`\n`;
        failCount++;
      }
    }

    const maxLength = 4000;
    if (report.length > maxLength) {
      const lines = report.split('\n');
      let currentChunk = '';
      let partNumber = 1;
      const totalParts = Math.ceil(report.length / maxLength);
      
      for (const line of lines) {
        if ((currentChunk + line + '\n').length > maxLength && currentChunk !== '') {
          await bot.sendMessage(ownerChatId, `📦 ${botData[botToken].name} *Delivery Report (Part ${partNumber}/${totalParts}):*\n\n${currentChunk}`, {
            parse_mode: "Markdown",
          });
          currentChunk = line + '\n';
          partNumber++;
        } else {
          currentChunk += line + '\n';
        }
      }
      
      if (currentChunk) {
        await bot.sendMessage(ownerChatId, `📦 ${botData[botToken].name} *Delivery Report (Part ${partNumber}/${totalParts}):*\n\n${currentChunk}`, {
          parse_mode: "Markdown",
        });
      }
    } else {
      await bot.sendMessage(ownerChatId, `📦 ${botData[botToken].name} *Delivery Report:*\n\n${report}`, {
        parse_mode: "Markdown",
      });
    }

    await bot.sendMessage(ownerChatId, `📊 ${botData[botToken].name} *Summary:* ✅ ${successCount} successful, ❌ ${failCount} failed`);
  }
}

// Express server setup
const express = require("express");
const app = express();
const PORT = process.env.PORT || 3000;

app.get("/", (req, res) => {
  res.send(`<!DOCTYPE html>
<html>
<head>
    <title>Dual Telegram Bots</title>
    <style>
        body { font-family: Arial, sans-serif; margin: 40px; text-align: center; }
        .bot-status { margin: 20px; padding: 20px; border-radius: 10px; background: #f0f0f0; }
        .online { background: #d4edda; }
        .offline { background: #f8d7da; }
    </style>
</head>
<body>
    <h1>🤖 Dual Telegram Bots Running</h1>
    ${botConfigs.map((config, index) => `
        <div class="bot-status ${bots[index] ? 'online' : 'offline'}">
            <h2>${config.name}</h2>
            <p>Status: ${bots[index] ? '✅ Online' : '❌ Offline'}</p>
            <p>Username: @${config.username}</p>
        </div>
    `).join('')}
    <p>Total bots running: ${bots.length}/${botConfigs.length}</p>
</body>
</html>`);
});

app.listen(PORT, () => {
  console.log(`🌐 Server running at http://localhost:${PORT}`);
  console.log(`🤖 Running ${bots.length} bots simultaneously!`);
});